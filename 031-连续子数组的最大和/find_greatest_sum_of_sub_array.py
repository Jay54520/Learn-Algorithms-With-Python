...
class Solution:
    def FindGreatestSumOfSubArray(self, array):
        ...
        * 返回最大和，结束
        ...
        时间复杂度：O(n)，遍历一次数组为 O(n)，在每次循环中只是常数级操作，所以为 O(n)
        空间复杂度：O(1)，只需要储存最大和
        ...
        * 数组有正数有负数
        ...
        1.  array[i]，当 i = 0 或 f(i - 1) < 0
        2. f(i - 1) + array[i]，当 i > 0 且 f(i - 1) >= 0

        对于 1，
            当 i = 0 时，只有一个子数组——array[0]，所以此时的最大和为 array[i]；
            当 f(i - 1) < 0 时，由于加上 f(i - 1) 会使 f(i) 变小，所以此时的最大和为 array[i]；
