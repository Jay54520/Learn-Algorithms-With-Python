...
class Solution:
    def NumberOf1Between1AndN_Solution(self, n):
        ...
        * 含有 1 个 1，任意一位为 1，其他为位 [0, 1) 或 (1, 9]，有 1 * C(n, 1) * 9 ^ (n - 1) 个 1
        * 含有 2 个 1，任意两位为 1，其他为位 [0, 1) 或 (1, 9]，有 2 * C(n, 2) * 9 ^ (n - 2) 个 1
        * ...
        * 含有 n 个 1，任意 n 位为 1，其他为位 [0, 1) 或 (1, 9]，有 n * C(n, n) * 9 ^ (n - n) 个 1

        由于要求小于等于 n，所以要排除大于 n 的含有 1 的数。遍历 (n, 10 ^ digit - 1)，从总次数中减去其中 1 的次数
